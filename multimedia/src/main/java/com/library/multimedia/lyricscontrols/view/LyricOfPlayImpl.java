 ...
 *
 */
public class LyricOfPlayImpl extends View implements ILrcView {
    /**
     * 歌词的最大缩放比例
     **/
    ...
    public LyricOfPlayImpl(Context context) {
        super(context);

        init(context);
    }

    public LyricOfPlayImpl(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
        ...
        for (int i = minRaw; i <= maxRaw; i++) {
            ...
            if (i == mCurRow) {//画高亮歌词
                ...
                if (textWidth > getWidth()) {
                    ...
                    float textX = (getWidth() - textWidth) / 2;
                    canvas.drawText(text, (getWidth() - mPaintForOutLine.measureText(text)) / 2, rowY, mPaintForOutLine);
