...
public interface GIftRankContract {
    //对于经常使用的关于UI的方法可以定义到BaseView中,如显示隐藏进度条,和显示文字消息
    interface View extends BaseView {
        //设置排行榜
        void setRecyclerView(GIftRankAdapter adapter);

        //专辑设置界面数据
        void fillDataToView_Album(AlbumDetailResponse response);

        //单曲设置界面数据
        void fillDataToView_Single(GIftRankResponse response);
    ...
    interface Model extends IModel {
        ...
        //获取礼物排行榜
        Observable<GIftRankResponse> getGiftRank(Map<String, String> info);
