...
import static com.clicktech.snsktv.module_discover.ui.helper.TemplateAndFilterType.FRONTBACK;
import static com.clicktech.snsktv.module_discover.ui.helper.TemplateAndFilterType.FULLSCREEN;
...
import static com.clicktech.snsktv.module_discover.ui.helper.TemplateAndFilterType.STANDARD;
...
@ActivityScope
public class MvPreviewSecondPresenter extends BasePresenter<MvPreviewSecondContract.Model, MvPreviewSecondContract.View> {
    private final TemplateAndFilterType[] mTemplateTypes = new TemplateAndFilterType[]{
            STANDARD,//左右
            FRONTBACK,//前后
            FULLSCREEN,//独占
    };
    ...
    private TemplateAndFilterAdapter mTemplateAdapter;
    private TemplateAndFilterAdapter mFilterAdapter;
    private TemplateAndFilterAdapter.onFilterChangeListener onTemplateChangeListener =
            new TemplateAndFilterAdapter.onFilterChangeListener() {
                @Override
                public void onFilterChanged(int position) {
                    mRootView.setTemplateEffect(mTemplateTypes[position]);
                }
            };
    ...
    @Inject
    public MvPreviewSecondPresenter(MvPreviewSecondContract.Model model, MvPreviewSecondContract.View rootView
    ...
    public void requestData(final SongInfoBean singinfo) {
        PermissionUtil.recordmv(new PermissionUtil.RequestPermission() {
            @Override
            public void onRequestPermissionSuccess() {
                downloadLyric(singinfo.getLyric_url());
            }
        ...
        mTemplateAdapter = new TemplateAndFilterAdapter(mApplication, mTemplateTypes, 2);
        mFilterAdapter = new TemplateAndFilterAdapter(mApplication, mFilterTypes, 0);
        mTemplateAdapter.setOnFilterChangeListener(onTemplateChangeListener);
        mFilterAdapter.setOnFilterChangeListener(onFilterChangeListener);
        mRootView.setTemplateFilterAdapter(mTemplateAdapter, mFilterAdapter);
    }
     ...
     * @param lyricpath
     */
    private void downloadLyric(String lyricpath) {
