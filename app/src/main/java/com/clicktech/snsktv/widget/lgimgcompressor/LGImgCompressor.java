package com.clicktech.snsktv.widget.lgimgcompressor;
...
//        File file = new File(Environment.getExternalStorageDirectory().getPath(), "LGImgCompressor/Images");
        File file = new File(Environment.getExternalStorageDirectory().getPath(), "KM/IMG");
        if (!file.exists()) {
            file.mkdirs();
        }
    ...
    public void starCompressWithDefault(String srcImageUri) {
        new CompressTask().execute(srcImageUri, "" + DEFAULT_OUTWIDTH, "" + DEFAULT_OUTHEIGHT, "" + 100);
    }
