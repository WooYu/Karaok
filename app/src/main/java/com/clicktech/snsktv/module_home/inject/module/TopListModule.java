...
import com.clicktech.snsktv.module_home.model.TopListModel;
...
public class TopListModule {
    ...
    /**
     * 构建TopListModule时,将View的实现类传进来,这样就可以提供View的实现类给presenter
     *
     * @param view
     */
    ...
    @ActivityScope
    @Provides
    TopListContract.View provideTopListView() {
        return this.view;
    }

    @ActivityScope
    @Provides
    TopListContract.Model provideTopListModel(ServiceManager serviceManager, CacheManager cacheManager
            , Gson gson, Application application) {
        return new TopListModel(serviceManager, cacheManager, gson, application);
    }
}