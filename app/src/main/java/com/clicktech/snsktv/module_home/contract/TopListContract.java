...
public interface TopListContract {
    ...
    interface View extends BaseView {
        //设置推荐列表的RecyclerView
        void setTopListRecyclerView(TopListAdapter adapter);

        //设置往期专题的RecyclerView
        void setSpecialRecyclerView(SpecialAdapter adapter);
    }

    //Model层定义接口,外部只需关心model返回的数据,无需关心内部细节,及是否使用缓存
    interface Model extends IModel {
        //获取推荐列表
        List<TopListEntity> getTopList();

        //获取往期专题
        List<PastSpecialEntity> getPastSpecialList();
    }
}