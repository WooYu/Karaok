...
import com.clicktech.snsktv.module_home.model.SongListModel;
...
public class SongListModule {
    ...
    /**
     * 构建SongListModule时,将View的实现类传进来,这样就可以提供View的实现类给presenter
     *
     * @param view
     */
    ...
    @ActivityScope
    @Provides
    SongListContract.View provideSongListView() {
        return this.view;
    }

    @ActivityScope
    @Provides
    SongListContract.Model provideSongListModel(ServiceManager serviceManager, CacheManager cacheManager
            , Gson gson, Application application) {
        return new SongListModel(serviceManager, cacheManager, gson, application);
    }
}