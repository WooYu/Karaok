...
public interface MineContract {
    ...
    interface View extends BaseView {
        //更新用户信息
        void updateUserInfoView(MineUserInfoResponse response);

        //更新用户相册
        void updateAlbumOfUser(List<String> urls);

        //设置礼物列表数据到关注和热门页面
        void setGiftListForChildFragment(GiftListResponse response);
    }

    //Model层定义接口,外部只需关心model返回的数据,无需关心内部细节,及是否使用缓存
    interface Model extends IModel {
        //获取相册
        Observable<SingerAlbumResponse> getSingerAlbum(Map<String, String> info);

        //获取用户信息
        Observable<MineUserInfoResponse> getUserInfo(Map<String, String> info);
